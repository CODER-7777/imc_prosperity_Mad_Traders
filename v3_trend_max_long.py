...
class Trader:
    """
    V3 - Maximum Profit Execution
    - Osmium: Safe mean-reversion around 10000.
    - Pepper Root: Abuses the 1,000 point/day trend by taking and holding maximum long position.
    """
    def __init__(self):
        self.limits = {
    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                # STRATEGY: Trend is perfectly linear +1000/day.
                # Do NOT sell. Accumulate max long and ride it to the moon.
                buy_cap = limit - position
                
                # Market take any current sells (it will always go up anyway)
                for ask, ask_vol in sorted(order_depth.sell_orders.items()):
                    if buy_cap <= 0:
                        break
                    available = -ask_vol
                ...
                # Place limit order for the rest slightly above best bid to get filled fast
                if buy_cap > 0:
                    orders.append(Order(product, best_bid + 1, buy_cap))
                    
                # NO SELL ORDERS! Never go short against a strictly drifting trend.
